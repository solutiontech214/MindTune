...
class GaanaAPI {
 ...
 async getStreamUrl(trackId: string): Promise<string> {
  try {
    ...
    const streamUrl = fallbackStreams[trackId];
    if (streamUrl) {
      console.log(`Resolved stream URL for ${trackId}:`, streamUrl);
      return streamUrl;
    }

    console.warn(`Stream URL not found for trackId: ${trackId}`);
    return "";
  } catch (error) {
    console.error("getStreamUrl failed:", error);
    return "";
  }
}
